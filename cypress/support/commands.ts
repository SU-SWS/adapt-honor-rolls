...
import { maxDataUser } from '../../src/services/megaprofile/mocks/mock-data/constituents';
...
Cypress.Commands.add('login', (username: string = '') => {
  ...
  const userToken = {
    userName: user.contact.name.username,
    email: user.emails?.[0]?.emailAddress,
    firstName: user.contact.name.fullNameParsed.firstName,
    lastName: user.contact.name.fullNameParsed.lastName,
    SUID: 'NOTHINGTOSEEHERE',
    encodedSUID: user.encodedSUID,
  };
  ...
  cy.task('signToken', userToken).then((token) => {
    ...
    cy.setCookie('honor-rolls-auth-session', 'active');
    cy.setCookie('honor-rolls-auth', token as string);
  });
});
