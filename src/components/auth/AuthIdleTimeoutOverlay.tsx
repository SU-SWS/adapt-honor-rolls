...
export const AuthIdleTimeoutOverlay = ({
  ...
  const logout = (): void => window.location.replace(routes.logout());
  const timeoutLogout = (): void => window.location.replace(routes.logout('auth_timeout'));
  ...
  const startModalTimer: () => void = useCallback(() => {
    setOpen(true);
    modalTimer.start();
  }, [setOpen, modalTimer]);
  ...
  const duration: (string | undefined) = useMemo(() => {
    if (modalTimer.isRunning) {
      const mins: number = Math.ceil(modalTimer.remainingTime / (1000 * 60));
      const secs: number = Math.floor(modalTimer.remainingTime / 1000);
