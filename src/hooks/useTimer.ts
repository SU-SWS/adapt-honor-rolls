...
export const useTimer = ({
  ...
  const clearTicker = (): void => clearTimeout(tickTimeoutRef.current);
  const clearTimer = (): void => clearTimeout(timeoutRef.current);
  ...
  const getElapsedTime: () => number = useCallback(() => elapsedTime, [elapsedTime]);
  const getRemainingTime: () => number = useCallback(() => timeout - getElapsedTime(), [timeout, getElapsedTime]);
